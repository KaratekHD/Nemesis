...
#
# This file is stolen from https://github.com/SensiPeeps/skyleebot
...
def kang(update, context):
    ...
    args = context.args
    packnum = 0
    packname = "beta2" + str(user.id) + "_by_" + context.bot.username
    packname_found = 0
    max_stickers = 120
    ...
    try:
        ...
        if png_sticker:
            success = context.bot.create_new_sticker_set(
                user.id,
                packname,
                f"{context.bot.first_name} pack by {name}" + extra_version,
        ...
        if tgs_sticker:
            success = context.bot.create_new_sticker_set(
                user.id,
                packname,
                f"{context.bot.first_name} animated pack by {name}" + extra_version,
                f"{context.bot.first_name} animated pack by {name}" + extra_version,
