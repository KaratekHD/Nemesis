...
def export_data(bot: Bot, update: Update):
    with BytesIO(str.encode(json.dumps(helper.export_data(update.effective_chat, bot)))) as output:
        output.name = str(update.effective_chat.id) + ".toml"
        update.effective_message.reply_document(document=output, filename=str(update.effective_chat.id) + ".json",
